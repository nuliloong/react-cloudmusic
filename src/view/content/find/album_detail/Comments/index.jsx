...
export default function Comments({ id, setCommentCount }) {
  ...
  const getComments = async (pageIndex) => {
    const [err, res] = await to(getAlbumComment({ id, offset: (pageIndex - 1) * 30, limit: 30 }))
    if (res) {
  ...
  const clickLike = async (item) => {
    ...
    const [err, res] = await to(isLikeComment({ id, cid: item.commentId, t, type: 3 }))
    if (res?.code === 200) {
  ...
  const comment = async () => {
    ...
    const [err, res] = await to(replyComment({ t, id, commentId, type: 3, content: textValue }))
    if (res && res.code === 200) {
      setTextValue("")
